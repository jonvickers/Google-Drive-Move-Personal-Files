...
var sourceFolderId = "SOURCE DRIVE FOLDER ID"
var targetFolderId = "TARGET DRIVE FOLDER ID"
var deletedFolderId = "DELETED DRIVE FOLDER ID" //Folder for non-owned files to move them,  Cannot be below the source folder or you will have problems.  Must be at the same hierarchical level as the source or above it, do not put it below.
var copyTheSourceFolder = false;  //do you want it to actually move the source folder as a sub folder into the target
//var emailsToAlwaysCopy = "";
var emailsToAlwaysCopy = ""; //csv of emails that you cannot move and want to make a copy and delete
var mode = "move" // move | copy | collect (collect names only) 

//Values to Ignore
var MAX_RUNNING_TIME_MS = 4.5 * 60 * 1000; //I would not set higher than 5 for 5 minutes.  Scripts timeout at 6 minutes and you should not allow it to timeout without storing the keys.
...
function ProcessFiles(){
  ...
  if(emailsToAlwaysCopy == undefined || emailsToAlwaysCopy == ""){
    ...
    //JV 7/28/2023 removed because its sending emails to any editors
...
function processRootFolder(rootFolder) {
  ...
  while (recursiveIterator.length > 0) {
    ...
    if (timeLimitExceeded & !debug) {
      ...
      console.warn("Stopping loop after '%d' milliseconds. Please continue running.", elapsedTimeInMS);
      return;
    }
  }
...
function processFile(file, path, sourcefolderArray) {
  ...
  if(emailsToAlwaysCopy == null){
    isOnFileCopyList = false;
  }
  else if(emailsToAlwaysCopy.includes(fileOwnerEmail)){
  ...
  if(mode==="copy"){
    ...
    if (targetFolder.getFilesByName(file.getName()).hasNext() === false){
      if(isFileOwner){
        if(!debug){
          ...
          //in debug mode , make a copy
          //this will rename the file prefix Copy of
      ...
      if(isOnFileCopyList || isFileOwner){
        file.setName("Delete-CopiedSkipped-"+ path.replace("/","-") + "-"+file.getName());
      }
      if(!debug){
        file.moveTo(deletedFolder);        
      }
    ...
    throw new Error("You must specify mode as move or copy or collect");
    //technically mode='collect' should never get down this far.
  }
...
function considerDeleteFolder(folder){
  if(folder.getFiles().hasNext()){
    console.log("Source Folder "+folder.getName()+" has files - cannot delete.");
    return;
  }else{
    console.log("Source Folder "+folder.getName()+" does NOT have files.");
    //test to see if any sub folders
    if(folder.getFolders().hasNext()){
      console.log("Source Folder "+folder.getName()+" has sub folders - cannot delete.");
      return;
    }else{
      console.log("Source Folder "+folder.getName()+" does NOT have sub folders.");
      ...
      if(folder.getOwner().getEmail() === currentUserEmail){
        ...
        if(!folder.getName().includes("Delete")){
        console.log("Renaming Folder to Delete");
        folder.setName("Delete-"+folder.getName());
        }
      }
    }
  }
